...
import { fetchMessages, selectAllMessages } from '../../slices/messagesSlice.js'
...
const MainPage = () => {
  ...
  const { data, isLoading, error, status } = useGetChannelsQuery()

  console.log(status)

  const channels = data

  const activeChannelId = useSelector(selectActiveChannelId) || '1'

  const activeChannel = data?.find(c => c.id === activeChannelId)
